...
! BAS
  ...
  type aoflux_in_type
     ...
     real(R8) , pointer :: up2_sfc     (:) => null() ! u sfc wind variance from CLUBB
     real(R8) , pointer :: vp2_sfc     (:) => null() ! v sfc wind variance from CLUBB
...
! --- BAS
!    call NUOPC_CompAttributeGet(gcomp, name='add_clubb_gusts', value=cvalue, isPresent=isPresent, isSet=isSet, rc=rc)
!    if (chkerr(rc,__LINE__,u_FILE_u)) return
!    if (isPresent .and. isSet) then
!       read(cvalue,*) add_clubb_gusts
!    else
!       add_clubb_gusts = .false.
!    end if
    add_clubb_gusts = .true.
! --- BAS
  ...
  subroutine med_aofluxes_update(gcomp, aoflux_in, aoflux_out, rc)
    ...
    call flux_atmocn (logunit=logunit, &
         ...
         rainc=aoflux_in%rainc, up2_sfc=aoflux_in%up2_sfc, vp2_sfc=aoflux_in%vp2_sfc, & !BAS
         ...
         add_gusts=add_gusts, add_clubb_gusts=add_clubb_gusts, & !BAS
...
!================================================================================
  ...
  ! --- BAS
       if (add_clubb_gusts) then
          call fldbun_getfldptr(fldbun_a, 'Sa_up2_sfc', aoflux_in%up2_sfc, xgrid=xgrid, rc=rc)
          if (chkerr(rc,__LINE__,u_FILE_u)) return
          call fldbun_getfldptr(fldbun_a, 'Sa_vp2_sfc', aoflux_in%vp2_sfc, xgrid=xgrid, rc=rc)
          if (chkerr(rc,__LINE__,u_FILE_u)) return
       end if
  ! --- BAS
    end if
  ...
  subroutine set_aoflux_out_pointers(fldbun, lsize, aoflux_out, xgrid, rc)
    ...
    if (add_gusts .or. add_clubb_gusts) then  ! BAS
